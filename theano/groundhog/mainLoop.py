...
class MainLoop(object):
    ...
    def save(self):
        ...
        numpy.savez(self.state['datafolder']+self.state['prefix']+'timing.npz',
                    **self.timings)
        if self.state['overwrite']:
            self.model.save(self.state['datafolder']+self.state['prefix']+'model.npz')
        else:
    ...
    def load(self, model_path=None, timings_path=None):
        if model_path is None:
            model_path = self.state['datafolder']+self.state['prefix'] + 'model.npz'
        if timings_path is None:
            timings_path = self.state['datafolder']+self.state['prefix'] + 'timing.npz'
        try:
...
#!!!!! [vulnerable code] The following 7 lines are self-augmented in case of increasing maximum iterations !!!
            ...
            for key in timings.keys():
                if not key in ['next_offset','step'] and timings[key].shape[0]<self.timings[key].shape[0]:
                    tt=numpy.zeros((self.timings[key].shape[0]),dtype='float32')
                    tt[0:timings[key].shape[0]]=timings[key]
                    timings[key]=tt
    ...
    def main(self):
        ...
        self.whole_time=self.timings['whole_time'][self.step-1] if self.step>0 else 0.0
        self.algo.whole_time=self.whole_time
        ...
        while (self.step < self.state['loopIters'] and
            ...
            try:                
                ...
                for name in rvals.keys():#record the training status of the algorithm
